...
42. **Aucun prix inventé** (2026-08-05) — `priceCents` vaut `null` pour Creator et Pro, ce qui affiche « Bientôt » et « Pas encore ouvert » au lieu d'un bouton de paiement qui ne mène nulle part. Aucun encaissement n'est branché (Stripe reste à écrire, voir la checklist) : annoncer un tarif qu'on ne peut pas facturer serait à la fois une donnée structurée fausse pour Google et une promesse commerciale intenable. **Le montant est la seule chose qui reste à décider — c'est une constante à remplir dans `plans.ts`.**
...
- **Reporter les montants Creator dans `packages/shared/src/plans.ts`** (`priceCents` **et** `yearlyPriceCents`, en centimes) — les prix existent déjà dans Stripe depuis le 2026-08-10, il ne s'agit plus de décider mais de recopier. **C'est le dernier verrou avant qu'un bouton de paiement apparaisse** : tant que `priceCents` vaut `null`, la page affiche « Bientôt ». Le sélecteur mensuel/annuel reste caché tant que `yearlyPriceCents` est nul. Idem pour Pro, dont les prix Stripe restent à créer.
