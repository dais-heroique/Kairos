...
85. **`grant:role`/`grant:plan` déguisaient toute erreur en « compte introuvable »** (2026-08-11) — signalé par l'utilisateur : les deux comptes étaient visibles dans la console Firebase, le script disait pourtant « aucun compte Firebase Auth ». Le `catch` autour de `getUserByEmail` n'inspectait pas le code d'erreur — n'importe quel échec (mauvais projet, permissions, identifiants absents) ressortait avec le même message rassurant et faux.

    La cause réelle, une fois l'erreur affichée en clair : `GOOGLE_APPLICATION_CREDENTIALS` pointait vers un fichier **qui n'existait plus** à cet endroit — un deuxième téléchargement du même `.json`, renommé `… (1).json` par le navigateur. Et `findServiceAccountKey()` amplifiait le problème : elle ne retombe sur la recherche automatique dans `~/Downloads` **que si aucun chemin n'est fourni** — un chemin explicite mais périmé la faisait abandonner plutôt que chercher ailleurs, laissant l'Admin SDK échouer bien plus loin avec un `ENOENT` qui ne dit ni où chercher ni que la clé a probablement juste changé de nom.

    Deux correctifs, dans `load-env.ts` et les deux scripts : (1) un chemin explicite invalide retombe maintenant sur la recherche automatique, comme s'il n'avait pas été fourni ; (2) le projet Firebase réellement interrogé s'affiche avant tout appel, et toute erreur autre que `auth/user-not-found` s'affiche telle quelle plutôt que d'être traduite en un message générique. Vérifié en simulant le scénario exact (chemin explicite absent, vraie clé présente dans le dossier de recherche) : la résolution retrouve la bonne clé.

86. **Le paywall sous-vendait Pro de moitié à un compte gratuit** (2026-08-14) — signalé par l'utilisateur : « le plan gratuit a plus de bénéfices en dessous que le pro ou creator, ça débloque presque rien ». `newCapabilitiesOf(plan.slug)` calculait toujours la différence avec le **palier juste en dessous**, jamais avec ce que le compte détient réellement. Un Radar qui bute sur `rankingArchive` (propre à Pro) ne voyait que les 4 capacités de Pro — jamais les 4 de Creator qu'il gagne aussi en sautant directement dessus. Pro paraissait apporter moins, pour 39 €/mois, que ce que Radar affiche déjà gratuitement juste à côté.

    `newCapabilitiesOf` reste tel quel — il sert `/tarifs`, où chaque colonne compare légitimement au palier précédent, dans une échelle qu'on parcourt dans l'ordre. `capabilitiesGainedFrom(from, to)` est la nouvelle fonction, utilisée uniquement là où le point de départ réel de l'utilisateur compte : le paywall lit `entitlements.effectivePlan` plutôt que de recalculer un adjacent. Un repère « N fonctionnalités de plus qu'aujourd'hui » a été ajouté au paywall, sur le modèle du « N fonctionnalités au total » de `PlanCards` — sans lui, un palier payant peut toujours paraître plus pauvre que le gratuit affiché à côté, même avec la bonne liste.
...
88. **Diagnostiqué grâce aux journaux activés en #87** (2026-08-14) — la trace complète, en production : `JSON.parse (<anonymous>) → getAccessToken → readUser → handleCheckout`. `FIREBASE_SERVICE_ACCOUNT` n'est pas un JSON valide, très probablement tronqué au moment du collage (le geste que `docs/STRIPE.md` § 4 bis décrit déjà comme « le seul moment un peu pénible au doigt »). Sans les journaux, ce diagnostic aurait été impossible à distance — la preuve que #87 n'était pas un réglage cosmétique.

    Le message d'erreur ne disait, lui, que « JSON.parse (<anonymous>) » — aucune piste. `getAccessToken` (`firestore-rest.ts`) rapporte désormais la **longueur** de la chaîne reçue : `0` tranche « le secret n'a jamais été enregistré, ou sous un autre nom », une valeur basse mais non nulle tranche « collage tronqué ». Jamais le contenu lui-même journalisé — c'est une clé privée. Correction en attente côté utilisateur : régénérer la clé (Console Firebase → Comptes de service → Générer une nouvelle clé privée) et recoller le fichier **en entier**.

89. **Vérification du jeton : dire contre quel projet il a été émis** (2026-08-14) — après #88, un deuxième message opaque : « Jeton refusé : unexpected "iss" claim value ». `apps/stripe-worker/wrangler.toml` et `apps/web/.env.production` disent tous deux `kairos-on` — le dépôt est cohérent avec lui-même, donc soit le site en ligne n'a pas cette valeur (déploiement pas à jour), soit une variable en double sur Cloudflare écrase celle de `wrangler.toml` (même classe de piège que les prix, décision #66). Deviner laquelle sans y avoir accès n'aurait aidé personne.

    `unverifiedClaims` (`verify-token.ts`) décode `iss`/`aud` **sans vérifier la signature** — uniquement pour enrichir le message d'erreur, jamais pour une décision d'accès. Ce ne sont pas des secrets : de simples identifiants de projet, déjà publics dans la configuration Firebase servie à tout visiteur. Le message dit désormais explicitement pour quel projet le jeton a été émis et lequel le Worker attend, plutôt que de renvoyer tel quel le message générique de `jose`. L'enrichissement ne s'ajoute que si `iss`/`aud` diffèrent réellement de l'attendu — sur une signature invalide ou un jeton expiré, répéter « attendu kairos-on, reçu kairos-on » n'aiderait personne. 3 tests sur la fonction pure ; `verifyFirebaseToken` elle-même reste non testée unitairement (JWKS distant, non mocké).

90. **Troisième secret, même famille de panne : `STRIPE_SECRET_KEY`** (2026-08-14) — après #88 (FIREBASE_SERVICE_ACCOUNT) et #89 (projet Firebase), la lecture Firestore et la vérification du jeton passaient enfin toutes les deux : l'erreur s'est déplacée à `stripeClient`, avec la trace `Stripe._setAuthenticator → new Stripe → stripeClient → handleCheckout`. Vérifié dans le code source du SDK (`node_modules/stripe/cjs/stripe.core.js`) plutôt que supposé : `_setAuthenticator` lève `'Neither apiKey nor config.authenticator provided'` dès que la clé passée est une chaîne vide — message qui ne nomme ni KAIROS ni le secret en cause.

    Garde explicite ajoutée dans `stripeClient` (`index.ts`), sur le même principe que #88 et #89 : dire ce qui manque, jamais journaliser la valeur (une clé Stripe réelle commence par `sk_live_`/`sk_test_` et fait plus de 100 caractères — une clé vide se distingue immédiatement). `stripeClient` exportée et testée directement (2 tests) plutôt que seulement via le point d'entrée HTTP, qui exigerait de simuler tout Cloudflare Workers pour un contrôle qui ne dépend que de la présence d'une chaîne.

    Trois secrets, trois pannes de la même forme — collés à la main sur mobile, chacun a fini vide, tronqué, ou associé au mauvais projet. Aucun des trois n'était visible avant que les journaux (#87) existent : la conclusion qui reste, au-delà du correctif, c'est que la case « secrets » de `docs/STRIPE.md` § 4 bis mérite d'être vérifiée un par un via `/health` et ces messages d'erreur plutôt que supposée correcte parce que « collée ».

91. **Migration vers Vercel amorcée, en parallèle de Firebase Hosting** (2026-08-14) — décision de l'utilisateur, après une session de débogage pénible sur trois secrets Cloudflare collés de travers (#88-90). Signalé avant d'agir : le plan Hobby (gratuit) de Vercel interdit contractuellement l'usage commercial, et Kairos encaisse déjà en mode réel — c'est très exactement la raison qui avait fait choisir Cloudflare Workers pour le paiement plutôt que Vercel (décision #65). Choix assumé : rester sur Hobby le temps de vérifier que tout fonctionne, passer au plan payant (20 $/mois) ensuite.

    **Aucune adaptation de code n'était nécessaire** — vérifié avant d'agir, pas supposé : le site est déjà 100 % statique sans `output: "export"` explicite (c'est Firebase qui décide de ne pas créer de Cloud Function quand zéro route est dynamique, via `"hosting": {"source": "apps/web"}` dans `firebase.json`), `turbo.json` déclare déjà `"dependsOn": ["^build"]` donc Vercel construit le graphe de dépendances (`packages/shared` → `core` → `payments` → `ai-gateway` → `web`) automatiquement via sa détection native de Turborepo, et `SITE_URL` était déjà surchageable par `NEXT_PUBLIC_SITE_URL`. Le réglage Vercel indispensable, vérifié par recherche plutôt que deviné : « Include source files outside of the Root Directory in the Build Step », sans quoi le build échoue en module introuvable sur les paquets internes.

    Adresse connue (`kairos-on.vercel.app`) : `ALLOWED_ORIGINS` dans `wrangler.toml` porte maintenant les trois origines (Firebase Hosting × 2, Vercel), sans retirer les deux premières — Firebase Hosting reste pleinement déployable en parallèle, rien n'est effacé.

    **Ce qui reste hors d'atteinte des outils disponibles ici** : poser les variables d'environnement du projet Vercel (`NEXT_PUBLIC_FIREBASE_*`, `NEXT_PUBLIC_STRIPE_*`) — l'intégration MCP Vercel expose la lecture (projets, journaux, analytics) et les achats, mais aucune écriture de variables de projet. Vérifié par deux recherches d'outils, pas supposé absent. Reste aussi à faire à la main : Firebase Console → Authentication → Authorized domains → ajouter `kairos-on.vercel.app`, sans quoi la connexion par lien email peut être refusée depuis ce domaine.

92. **Le changement de plan depuis `/compte` ouvre un paywall modal** (2026-08-17) — le compte Radar renvoyait vers `/tarifs`, ce qui cassait le parcours perçu par l'utilisateur et pouvait ramener à l'accueil selon le contexte de navigation. `SubscriptionCard` ouvre maintenant `PlanUpgradeModal` directement : croix de fermeture, touche Échap, clic sur l'arrière-plan, choix mensuel/annuel et deux plans payants dérivés du catalogue réel. Les boutons utilisent le même `SubscribeButton` que le reste du produit ; aucun secret ni logique de paiement n'est déplacé dans le navigateur. Le changement ne modifie ni Firebase, ni Cloudflare, ni la configuration d'hébergement.
...
94. **Codes owner à essai fixe et produits en observation visibles** (2026-08-17) — l’interface admin ne demande plus une durée d’essai variable : `createInviteCode` impose 5 jours pour chaque nouveau code, tandis que le nombre maximal d’utilisations reste réglable. Le dashboard conserve la règle de prudence à trois relevés pour les verdicts, mais affiche désormais explicitement les cinq produits en observation avec leur nombre réel de relevés, au lieu de les présenter comme une absence d’information. Aucun scraper n’est ajouté : la collecte reste celle des relevés administrateur documentés, afin de ne pas inventer de données ni introduire une source non autorisée ou payante.

95. **Création des codes d’invitation réservée à l’owner** (2026-08-17) — le rôle `admin` conserve l’accès aux opérations de catalogue et de pipeline, mais ne peut plus créer, désactiver ou supprimer de code d’invitation. L’interface masque ces actions aux admins non-owner et `firestore.rules` utilise `isSiteOwner()` pour les écritures sur `inviteCodes/*`. La rédemption par un utilisateur reste autorisée uniquement via la transaction contrôlée qui incrémente `usedCount`.

96. **Extension des marchés TikTok Shop** (2026-08-17) — `MARKETS` couvre désormais FR, US, UK, DE, IE, IT, ES, AT, BE, NL et PL. L’onboarding autorise plusieurs marchés et les écrans principaux lisent le premier marché choisi. Cette liste correspond aux marchés européens annoncés par TikTok en 2026, mais TikTok Shop fonctionne localement par pays : un marché ajouté dans le schéma n’a pas de catalogue Kairos tant qu’aucun relevé n’y a été collecté. La France reste le marché primaire et le fallback des anciens comptes. Source : https://newsroom.tiktok.com/tiktok-shop-expands-across-europe?lang=en-150.

97. **Détection et traduction de langue côté client** (2026-08-17) — l’export statique conserve le français comme contenu initial afin de ne pas introduire de middleware dynamique. `LanguageProvider` détecte la langue préférée du navigateur, applique le choix sauvegardé dans `localStorage` lorsqu’il existe, met à jour `html[lang]` et propose un sélecteur manuel. Les catalogues `en`, `de`, `es`, `it`, `nl` et `pl` reprennent toutes les clés du français ; les marchés belges et autrichiens utilisent respectivement néerlandais/français et allemand selon le choix du navigateur, avec possibilité de correction manuelle.

98. **Onboarding et période d’abonnement persistés** (2026-08-17) — l’écran final traduit désormais aussi son aperçu et ses erreurs de sauvegarde. `completeOnboarding` écrit `profile.onboardingCompletedAt` avec le profil ; une erreur Firebase reste visible au lieu de rediriger silencieusement. Les deux chemins de connexion redirigent les profils complétés vers `/tableau-de-bord`, les profils incomplets seulement vers `/onboarding/niches`. Le schéma `plan` ajoute `billingPeriod` avec fallback `null` pour les anciens documents. Le webhook Stripe écrit `monthly` ou `yearly` selon le prix réellement reçu, ainsi que `currentPeriodEnd`; une résiliation remet la période à `null`. La carte du compte affiche la période enregistrée et son statut traduit.

99. **Dossier juridique et paiement conditionné** (2026-08-17) — `docs/LEGAL_READINESS.md` rassemble les projets et la checklist de mise en conformité. Une page `/cookies` est publiée et reliée au footer et au sitemap. Les CGU/CGV mentionnent désormais la médiation à compléter, la politique de confidentialité ne prétend plus qu’une bannière cookies existe déjà, et les conditions d’affiliation ne promettent plus Stripe Connect ni un palier Ambassadeur non branché. Tout checkout configuré exige
 une confirmation explicite de lecture et d’acceptation des CGU/CGV avant l’ouverture de Stripe. Les informations d’entreprise, le médiateur, la TVA, les prestataires réellement activés et les mécanismes de consentement restent à confirmer avant la première vente.

100. **Audit production connecté du 2026-08-18** — l’onboarding en trois étapes écrit bien le profil puis redirige vers `/tableau-de-bord`. Les 90 produits récupérés via Apify apparaissent avec titres, photos, boutiques, prix et liens de fiche ; `/classements/produits` charge bien les 90 éléments après son état transitoire « Chargement… », et le simulateur calcule sur un produit réel. Les fiches restent volontairement prudentes avec un relevé et un historique insuffisants. Point critique à ne pas masquer : l’actor Apify utilisé est **US uniquement**, alors que l’interface par défaut du compte est FR ; ces données ne doivent donc pas être présentées comme un classement France. Aucune nouvelle collecte n’a été lancée pendant l’audit et aucune dépense Apify supplémentaire n’a été engagée.

101. **Cohérence i18n du compte et du paywall** (2026-08-18) — l’audit a révélé des chaînes françaises codées en dur dans `SubscriptionCard` et `PlanUpgradeModal`, visibles au milieu d’une interface anglaise lorsque `LanguageProvider` détecte `en`. Les textes de gestion d’abonnement, de portail Stripe, de période, de changement de plan et de fermeture du modal sont désormais traduits via `Account` en français/anglais ; les autres langues héritent du français tant qu’elles n’ont pas de copie dédiée. Typecheck et lint restent verts. La suite web simple conserve son test de budget dépendant de l’émulateur ; l’intégration n’a pas pu démarrer ici car le binaire Firebase CLI n’est pas installé.
...
- ~~**Les quatre prix**~~ **Complet le 2026-08-11** : Creator 19 €/190 €, **Pro 39 €/390 €**. Les quatre identifiants Stripe sont dans `wrangler.toml` (le Worker les lie, vérifié en `--dry-run`) et dans `.env.production`. Pro est fixé au **double** de Creator et pas plus : au-delà il faudrait une différence de nature (multi-comptes, API, accompagnement) que Pro n'a pas — il ajoute quatre fonctionnalités au même produit. La **remise annuelle est identique** aux deux paliers (−16 %), et un test l'impose : deux ratios différents donnent l'impression que les prix sont improvisés. Deux tests écrits pour tomber à ce moment précis (« Pro n'affiche pas de prix », « dit Bientôt ») ont été réécrits sur ce qui reste vrai — dont l'invariant qui compte : un abonné Creator qui bute sur une capacité Pro se voit proposer **Pro**, pas le palier qu'il a déjà.
